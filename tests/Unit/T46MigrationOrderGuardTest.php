...
it('migrations succeed normally once Spatie tables exist (no false positive)', function (): void {
    ...
    expect(\Illuminate\Support\Facades\Schema::hasTable('clr_role_meta'))->toBeTrue();
    expect(\Illuminate\Support\Facades\Schema::hasTable('clr_role_ctx'))->toBeTrue();
    expect(\Illuminate\Support\Facades\Schema::hasTable('clr_ctx_overrides'))->toBeTrue();
});
